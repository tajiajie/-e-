<template>
  <div>
    <div class="top">
      <MyHeader><router-link to="/Home" class="iconfont icon-fanhui"></router-link>制度建设</MyHeader>
    </div>
    <!--新闻-->
    <div class="news-list">
      <router-link :to="'/NEWS?id='+item._id" v-for="item in arr" :key="item.id" class="newsrow">
        <div class="news-listtu">
          <img :src="item.img" alt="">
        </div>
        <div class="newscol">
          <div class="newsnotice-title"></div><span v-text="item.title"></span>
...
<script>
  ...
  export default {
    ...
    mounted(){
      this.$axios.get("getZDJS").then(res=>{
